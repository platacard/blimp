...
public extension Blimp {
    ...
    struct Maintenance: Sendable {
        ...
        @discardableResult
        public func registerDevice(name: String, udid: String, platform: ProvisioningAPI.Platform) async throws -> ProvisioningAPI.DeviceRegistration {
            let registration = try await api.registerDevice(name: name, udid: udid, platform: platform)
            switch registration {
            ...
            case .alreadyRegistered(let device):
                logger.info("Device already registered: \(device.name) (\(device.id)), status: \(device.status)")
            }
            return registration
        }
